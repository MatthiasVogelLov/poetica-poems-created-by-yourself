  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${isScrolled ? 'py-4 bg-white/80 backdrop-blur-md shadow-sm' : 'py-6 bg-transparent'}`}>
      <div className="container-wide flex items-center justify-between">
        ...
        <div className="hidden md:flex space-x-8 items-center">
          <NavLink to="/generator" className={({isActive}) => `text-sm tracking-wide transition-all duration-200 hover:text-primary/80 ${isActive ? 'text-primary font-medium' : 'text-foreground/80'}`}>
            Gedicht erstellen
          </NavLink>
          
          <NavLink to="/impressum" className={({isActive}) => `text-sm tracking-wide transition-all duration-200 hover:text-primary/80 ${isActive ? 'text-primary font-medium' : 'text-foreground/80'}`}>
            Impressum
          </NavLink>
          
          <NavLink to="/datenschutz" className={({isActive}) => `text-sm tracking-wide transition-all duration-200 hover:text-primary/80 ${isActive ? 'text-primary font-medium' : 'text-foreground/80'}`}>
            Datenschutz
          </NavLink>
          
          <NavLink to="/agb" className={({isActive}) => `text-sm tracking-wide transition-all duration-200 hover:text-primary/80 ${isActive ? 'text-primary font-medium' : 'text-foreground/80'}`}>
            AGB
          </NavLink>
          
          <NavLink to="/kontakt" className={({isActive}) => `text-sm tracking-wide transition-all duration-200 hover:text-primary/80 ${isActive ? 'text-primary font-medium' : 'text-foreground/80'}`}>
            Kontakt
          </NavLink>
