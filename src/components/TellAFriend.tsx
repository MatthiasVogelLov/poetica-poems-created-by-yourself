  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('tell-a-friend', {
        body: {
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
            ...
            <Input id="email" placeholder="freund@example.com" type="email" value={email} onChange={e => setEmail(e.target.value)} />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="message">Nachricht</Label>
            <Textarea id="message" value={message} onChange={e => setMessage(e.target.value)} className="min-h-[120px]" />
          </div>
        </div>
        <DialogFooter>
